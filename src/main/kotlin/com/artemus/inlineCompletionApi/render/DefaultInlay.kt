...
class DefaultInlay(parent: Disposable) : ArtemusInlay {
    ...
    // TODO: Need to fix this so text after the inlay is in correct position
    override fun render(editor: Editor, completion: String, offset: Int) {
        val lines = Utils.asLines(completion)
        if (lines.isEmpty()) return
        val firstLine = lines[0]
        val endIndex = offset + firstLine.indexOf(completion)

        val instructions = determineRendering(lines, "")
    ...
    private fun renderAfterSuffix(
        ...
        val afterSuffixIndex = endIndex + completion.length
